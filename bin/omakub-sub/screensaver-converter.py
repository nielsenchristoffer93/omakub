...
# Curated vibrant multi-stop color gradients
GRADIENTS = [
    [(168, 85, 247), (59, 130, 246), (6, 182, 212)],                   # Purple -> Blue -> Cyan
    [(251, 191, 36), (249, 115, 22), (239, 68, 68), (217, 70, 239)], # Sunset Yellow -> Orange -> Coral -> Magenta
    [(132, 204, 22), (16, 185, 129), (20, 184, 166), (6, 182, 212)], # Lime -> Emerald -> Teal -> Cyan
    [(244, 63, 94), (225, 29, 72), (147, 51, 234), (79, 70, 229)],   # Coral -> Rose -> Purple -> Indigo
    [(0, 242, 254), (79, 172, 254), (99, 102, 241), (236, 72, 153)], # Electric Cyan -> Royal Blue -> Indigo -> Hot Pink
    [(255, 107, 107), (255, 230, 109), (78, 205, 196)],               # Coral -> Gold -> Aquamarine Mint
]
...
def interpolate_palette(palette, t):
    ...
    return (
        int(c1[0] + (c2[0] - c1[0]) * sub_t),
        int(c1[1] + (c2[1] - c1[1]) * sub_t),
        int(c1[2] + (c2[2] - c1[2]) * sub_t),
    )

def image_to_ascii(image_path, max_width=None, max_height=None, gradient_index=0):
    ...
    # 2. Compact aspect ratio scaling
    cw, ch = img.size
    raw_ratio = ch / cw
    aspect = raw_ratio * (1.2 if raw_ratio < 0.35 else 0.55)

    new_w = min(max_width, cw)
    new_h = int(new_w * aspect)
    if new_h > max_height:
        new_h = max_height
        new_w = int(new_h / aspect)

    new_w = max(10, min(max_width, new_w))
    new_h = max(4, min(max_height, new_h))

    img_resized = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
    r_pixels = img_resized.load()

    # 3. Dynamic contrast midpoint thresholding
    ...
    cutoff = min_lum + (max_lum - min_lum) * 0.48
...
if __name__ == "__main__":
    ...
    grad_idx = int(float(sys.argv[4])) if len(sys.argv) > 4 else 0

    print(image_to_ascii(img_path, w, h, grad_idx))
