...
BACKGROUND_ORG_PATH="${OMAKUB_PATH:-$HOME/.local/share/omakub}/themes/$OMAKUB_THEME_BACKGROUND"
...
if [ -f "$BACKGROUND_ORG_PATH" ]; then
  # Always overwrite destination so updated wallpapers in the repository take effect immediately
  cp -f "$BACKGROUND_ORG_PATH" "$BACKGROUND_DEST_PATH"
  touch "$BACKGROUND_DEST_PATH"

  # Force GNOME to re-read and apply the wallpaper
